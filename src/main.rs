...
impl Default for EnemySpawnTimer {
    fn default() -> Self {
        Self { timer: Timer::from_seconds(ENEMY_SPAWN_TIME, TimerMode::Repeating) }
    }
}
...
pub fn spawn_enemies(
    ...
    if let Ok(window) = window_query.get_single() {
        for _number_of_enemy in 0..NUMBER_OF_ENEMIES {
...
pub fn update_enemy_direction(
    ...
    let half_emeny_size = ENEMY_SIZE / 2.0;
    let x_min = 0.0 + half_emeny_size;
    let x_max = window.width() - half_emeny_size;
    let y_min = 0.0 + half_emeny_size;
    let y_max = window.height() - half_emeny_size;
...
pub fn update_score(score: Res<Score>){
    if score.is_changed() {
        println!("Score: {}", score.value.to_owned());
    }
}
